...
def test_hot_slot_mapping_tolerates_non_contiguous_page_ids():
    layout = KVLayout(80, 1, 16, "fp32", "fp32")
    pages = build_page_descriptors(
        [0, 80],
        bytes_per_token=layout.storage_bytes_per_token,
    ...
    pages = [replace_page(page, page_id=10 * page.page_id + 7) for page in pages]
    plan = HostMemoryPlan(
        total_budget_bytes=2 * 2**20,
        pinned_limit_bytes=256 * 2**10,
        bounce_limit_bytes=256 * 2**10,
    ...
    assert probe.hot_page_ids == {7, 17}
    k = torch.empty((16, 1, 16))
    v = torch.empty_like(k)
    for page in pages[:2]:
        assert not probe.get(page, k, v).hit
        probe.put(page, k, v)
    for page in pages[:2]:
        assert probe.get(page, k, v).hit
    probe.close()
    assert plan.snapshot().dram_cache_allocated_bytes == 0
