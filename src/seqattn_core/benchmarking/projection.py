...
def main() -> None:
    ...
    try:
        ...
        hidden_cpu = make_host_tensor((args.tokens, args.hidden_size), dtype, generator)
        ...
        def run_once():
            ...
            for start in range(0, args.tokens, staged_plan.q_chunk_tokens):
                ...
                attention_gpu = raw_attention_cpu[start:stop].to("cuda", non_blocking=True)
        ...
        attention_flops = (
            4 * args.heads * args.head_dim * sum(length * length for length in lengths)
        )
