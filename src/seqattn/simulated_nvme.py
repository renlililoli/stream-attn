...
class _BandwidthTimeline:
    ...
    def reserve(
        self, physical_bytes: int, *, arrival_seconds: float
    ) -> _TransferReservation:
        if physical_bytes < 0:
...
class _DirectionThrottle:
    ...
    def wait(self, physical_bytes: int) -> SimulatedIoDelay:
        ...
        try:
            reservation = self.timeline.reserve(
                physical_bytes, arrival_seconds=acquired
            )
...
def _with_delay(metrics: PageReadMetrics, delay: SimulatedIoDelay) -> PageReadMetrics:
    return replace(
        ...
        simulated_service_seconds=(
            metrics.simulated_service_seconds + delay.service_seconds
        ),
        ...
        simulated_physical_bytes=(
            metrics.simulated_physical_bytes + delay.physical_bytes
        ),
    )
...
class _SimulatedPageReader(PageReader):
    ...
    def read_q(self, page: PageDescriptor, out: torch.Tensor) -> PageReadMetrics:
        ...
        return _with_delay(
            metrics, self.device.throttle_read(metrics.physical_bytes)
        )
    ...
    def read_kv(
        ...
        metrics = self.reader.read_kv(
            page, k_out, v_out, k_scales_out, v_scales_out
        )
        return _with_delay(
            metrics, self.device.throttle_read(metrics.physical_bytes)
        )
...
class SimulatedPageSource(PageSource):
    ...
    def open_reader(
        ...
        return _SimulatedPageReader(
            self.source.open_reader(memory_plan, queue_depth), self.device
        )
...
class _SimulatedPageWriter(PageWriter):
    ...
    def write_page(self, page: PageDescriptor, data: torch.Tensor) -> PageReadMetrics:
        ...
        return _with_delay(
            metrics, self.device.throttle_write(metrics.physical_bytes)
        )
...
class SimulatedPageSink(PageSink):
    ...
    def open_writer(
        ...
        writer = self.sink.open_writer(
            layout, cu_seqlens, pages, memory_plan, queue_depth
        )
