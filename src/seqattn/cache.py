...
from .paging import KVLayout, PageDescriptor, align_up
...
class KVPageCache:
    ...
    def __init__(
        ...
        *,
        ...
        self.slot_bytes = align_up(
            2 * self.tensor_bytes + 2 * self.scale_bytes,
            64,
        ) if self.max_tokens else 0
        self.slot_count = (
            min(len(pages), capacity_bytes // self.slot_bytes)
            if self.slot_bytes
            else 0
        )
    ...
    def __enter__(self) -> "KVPageCache":
        return self
